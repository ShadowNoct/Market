const VERSION = "v4.6 Portfolio Icon Fix";
...
const iconUrls = {
  ...
  portfolio: "https://minecraft.wiki/images/Chest_JE4_BE2.png",
...
const iconFallbackUrls = {
  portfolio: "https://static.wikia.nocookie.net/minecraft_gamepedia/images/e/e9/Chest_JE4_BE2.png/revision/latest?format=original"
};
...
function icon(key) {
  ...
  const fallback = iconFallbackUrls[key] || "";
  const fallbackCode = fallback
    ? ' onerror="this.onerror=null;this.src=\'' + fallback + '\';"'
    : "";

  return '<span class="item-icon"><img src="' + url + '" alt="' + key + '" loading="lazy"' + fallbackCode + '></span>';
}
...
function tabs() {
  ...
  return '<div class="profile-label">Preview Player — real addon uses the player who clicked the computer:</div>' +
    ...
    names.map(function(name) {
      return '<button class="' + (name === activePlayer ? "active" : "") + '" data-action="switchPlayer" data-args="' + encodeURIComponent(name) + '">' + name + '</button>';
    }).join("") +
    '</div>';
}
...
function home() {
  ...
  setForm("StoneHaven Exchange",
    ...
    button("portfolio", "My Portfolio", "Personal shares and inventory preview", "portfolio") +
...
function stockPage(key) {
  ...
  setForm(s.name,
    ...
    '<div class="stat-grid">' +
      ...
      '<div class="stat"><small>Your Inventory Preview</small><strong>' + p.inventory[key] + ' ' + s.item + '</strong></div>' +
...
function portfolio() {
  ...
  setForm("My Portfolio",
    ...
    rows +
    '<div class="panel"><strong>Inventory Preview</strong><br>' +
